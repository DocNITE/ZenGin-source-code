...
oCGame :: oCGame()
{
	...
	loadNextLevel	= FALSE;
	watchnpc		= NULL;
	scriptStartup	= TRUE;  // [Moos] 5.12.2000 nach Ulfs Telephonseelsorge geändert
	...
	pl_lightval		= 5000.0f;
	wldTimer		= NULL;
	timeStep		= -1.0f;			// Ermöglicht Einzelbildschaltung
	singleStep		= FALSE;	
	// Info - Pool
	guilds			= NULL;
	infoman			= NULL;
...
/* ----------------------------------------------------------------------
	...
	26.08.2000	[Moos]	
                ExitFocusModes() eingefügt

    13.09.2000  [Moos]
                statischen ogame-Pointer bei Bedarf löschen
...
oCGame :: ~oCGame()
{
    ...
    zCSession::Done(); // [Moos] Besser wir erledigen das, bevor wir den ogame-Pointer löschen. ~zCSession hätte es sowieso erledigt.

    if (ogame==this)
        ogame=NULL;  // [Moos] Pointer hierrauf beseitigen, wäre sowieso ungültig
...
void oCGame :: WorldInit()
{
	...
	// und den Timer
	// FIXME : Timer nicht länger global !
...
// c) LOAD GAME     : SC behält Position aus Zen-File
{		
    ...
    playerVob->GetModel(); // [Moos] sorge dafür, dass das Model existiert, bevor der Spieler in die Welt kommt; sonst sitzt er mit der Hüfte im Mesh
	InsertPlayerIntoWorld(playerVob,initialStartMatrix);
	
	playerVob	->SetOnFloor			(startpos);
	playerVob	->ResetXZRotationsWorld ();             // [Moos] 20.12.00 Anpassung an Vob-Änderung
	playerVob	->SetCollDet			(TRUE);

	// Wenn der Player neu erzeugt wurde, muss hier wieder ein Release stattfinden,
	// Fire & Forget -> Den Vob kennt zCWorld und die oCWorld-Listen über (InsertPlayerIntoWorld)
...
// Initialize World with Standard - Values
{
	...
	if (game_leveltype==zBSP_MODE_INDOOR) {		
        ...
        zRELEASE(pl_light); // [Moos] Leakfix für Laden
...
void oCGame :: WriteSavegame (int slotnr, zBOOL saveGlobals)
{
	...
	if (saveGlobals) {
		...
		// komplettes SaveGame 
		// bei einem Levelwechsel darf dieser Teil nicht durchgeführt werden.
...
{
	// Checke auf fehlerhaft übriggebliebene Objekte
...
void oCGame :: InitNpcAttitudes()
{
	...
	if (player) {
		// Die persönlichen Attitüden initialisieren (aus Gildentabelle)
		zCListSort<oCNpc>* npcNode = GetGameWorld()->GetVobListNpcs()->GetNextInList();
		oCNpc* npc = NULL;
		while (npcNode) {
			npc		= npcNode->GetData();
		...
		if (npc && npc->GetHomeWorld()) {
			...
			if (npc->IsDead() && npc->GetAnictrl() && npc->GetModel()) {
				...
				if (mob && mob->GetFreePosition(npc,oldPos)) {	
					// Position setzen
					npc->SearchNpcPosition	(oldPos);				// Setzt Höhe und checkt auf Platz.
				}
...
void oCGame :: ChangeLevel(const zSTRING& levelpath, const zSTRING& startpoint)
{
	...
	// Beachte : auf OldPlayer wurde ein AddRef ausgeführt
...
//	zsound->StopAllSounds();
	...
	CamInit();    // [Moos] Nix da! Das macht den (im Moment nicht vorhandenen) BSP-Tree kaputt. Wird später bei EnterWorld() nachgeholt.
...
void oCGame :: Render ()
{
	...
	if (m_bWorldEntered)
	{
		...
		// nix für 2 sekunden rendern (precaching & auf boden plöppen der nsc's abwarten
...
//	xxx
	...
	oCVisualFX::SetupAIForAllFXInWorld();			// ai der spezialeffekte an oder ausschalten (von der dist. zur camera abhängig, nur unbewegliche oCVisualFX Objekte)
...
// Führe ObjectTagesablauf-Trigger von hour1 bis hour2 Zeit aus
{
	...
	while (currentObjectRoutine) {
		...
		if ((timeStart<=timeRtn) && (timeRtn<=timeEnd)) {
			// Innerhalb der TimeRange, ausführen
			if (rtn->type==0) {
...
{
	// Alle NSC durchsuchen und FullRegenerate durchführen
	oCNpc* npc = NULL;
	zCListSort<oCNpc>* npcNode = GetGameWorld()->GetVobListNpcs()->GetNextInList();
	while (npcNode) {
		npc		= npcNode->GetData();
...
void Game_CreateInstance ( zSTRING &s, zSTRING &msg )
{
	...
	if (index>=0) {
		...
		if (sym->type==zPAR_TYPE_INSTANCE) {
			...
			// unnötig: int mob		= parser.GetIndex(zSTRING("C_MOB"));
...
static int megamorzel = (591 * 999);    // Hidden VAR für Fingerprint
//////////////////////////////////////////////////////////////////////////
// Gothic2
// [BENDLIN] 1.28,			'303',		JoWooD, Goldmaster
// [BENDLIN] 1.28promo,		'7311',		JoWooD, Jürgen Schäpker - MARVIN mit 'MüCKE' oder 'MÜCKE'
//////////////////////////////////////////////////////////////////////////
// G2Addon
...
void Game_OpenSpeciesConsole()
{
	...
	game_species_con =  zNEW(zCConsole(0,1500,zSTRING("Spezies abhängige Konstanten")));
...
//	game_species_con -> InsertVar ( zSTRING("Schwimmzeit (ms)"),		zSTRING("SWIM"),	zCON_TYPE_FLOAT,&oCNpc :: player -> swimtime);							
	game_species_con -> InsertVar ( zSTRING("Wasserhöhe Knie"),			zSTRING("KNEE"),	zCON_TYPE_FLOAT,&oCNpc :: player -> GetAnictrl() -> config.zMV_WATER_DEPTH_KNEE);
	game_species_con -> InsertVar ( zSTRING("Wasserhöhe Brust"),		zSTRING("CHEST"),	zCON_TYPE_FLOAT,&oCNpc :: player -> GetAnictrl() -> config.zMV_WATER_DEPTH_CHEST);
// CODECHANGE [MICHAEL]
...
// CODECHANGE [MICHAEL] END
	game_species_con -> InsertVar ( zSTRING("Stufenhöhe"),				zSTRING("STEP"),	zCON_TYPE_FLOAT,&oCNpc :: player -> GetAnictrl() -> config.zMV_STEP_HEIGHT);							
...
void Game_OpenFightConsole()
{
	...
	game_fight_con -> InsertVar ( zSTRING("AutoAim Höhenwinkel"),		zSTRING("UPANGLE"), zCON_TYPE_INT,	&oCNpc :: angle_far_up);

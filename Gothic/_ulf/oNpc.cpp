...
// Häufig gebrauchte String-Konstanten
...
/* ----------------------------------------------------------------------
    ...
    die Vobtree-Problematik. Rückgabawert muss für SafeRelease aufgehoben werden.
...
/* ----------------------------------------------------------------------
    ...
    die Vobtree-Problematik. Zweites Argument muss der Rückgabewert
    von SafeAddRef() sein.
...
public:
	...
	zBOOL DontDelete(){
        // Lösche ihn nicht, wenn er noch in der Welt oder der letzte seiner Art (ModelPrototype) ist.
...
void oCNpc::Shrink(){
    if (!ogame)  // [Moos] whoops. Das sieht gefährich aus. Wahrscheinlich sind wir schon beim Abräumen der statischen Objekte.
        return;
...
// [EDENFELD] 1.09 Übertrag aus der Sequel Codebase
// könnte einen Bug fixen, der sich darin äüsserte, das Overlays teilweise einfach 
// entfernt wurden
//	[Bendlin] 2001-10-23
//
//	Hier wurde das Hash-Template für zSTRING mit zTOverlayIndex als Parameter überschrieben
//	(mal davon abgesehen, das dies von VC eigentlich nicht kompiliert werden dürfte...),
//	gehe ich davon aus, das hier beabsichtigt war, das Hash-Template für zTOverlayIndex
//	zu überschreiben, welches im zCSparseArray von zTOverlayIndex benutzt werden soll.
...
oCNpc :: oCNpc ()
{
    ...
    SetCollisionClass		(oCCollObjectCharacter::S_GetCollObjClass());           // [Moos] 20.12.00 Anpassung an Vob-Änderung
	SetCollDet				(TRUE);
/*
...
// Destuctor
{
    ...
    // [Moos] Mache die ActiveInfo unschädlich
...
//    activeInfo->npc = NULL;
    ...
    if (player == this)
        player = NULL;   // [Moos] oCNpc::player darf nicht auf eine ungültige Adresse zeigen
...
//	if (instanceName=="BAU_902_POCK01") parser.ShowCode(nr);
	...
	*/
	...
	// für den Fall das NpcType NPC_FLAG_GHOST enhält
...
void oCNpc :: SetAttitude(int wert)
{
	...
	// FIX ab 1.13a: temporäre Attitude muss beim Setzen auch berücksichtigt werden
	// doch wieder raus...
	//SetTmpAttitude(wert);
}
...
void oCNpc :: SetToMad(zREAL ms)
{
	...
	// Wechsel nur wirklich vollführen, wenn NSC 
...
void oCNpc :: CheckAngryTime()
{
	if (tmp_attitude != attitude) {
		...
		// Temporäre Attitüde aktiv, runterzählen
		attTimer -= ztimer.GetFrameTimeF();
		if (attTimer<=0) {
			// Attitude verbessert sich...
			tmp_attitude	= attitude;
			// Nochmal aufrufen, damit wieder richtige Std-FaceAni läuft
...
// TEMP für Alex zum schnellen Aktivieren von Kampftalenten !!!
{
	int tal;
...
void oCNpc::SetTalentSkill(int talentNr, int skill)
{
	...
	if (learnSkills)
	{
		...
		if (setFightMode>=0)   // [Moos] ApplyOverlay kümmert sich nun um die Details // && GetModel()) 
		{
			zSTRING ws = oCAniCtrl_Human::GetWeaponHitString(setFightMode);
...
// Check bei Waffen und rüstbaren Teilen
{
...
/* ----------------------------------------------------------------------
    ...
    Stellt das Nicht-Benutzen-Können eines Items dar
...
//	zCView :: StdPrintwin (zSTRING("< Die Voraussetzungen zum Benutzen dieses Items sind nicht erfüllt. >\n"));
...
/* ----------------------------------------------------------------------
    ...
    Überprüft alle equippten Items, ob die Voraussetzungen noch
    erfüllt sind.
    ...
    while (!finished)
    {
    	...
    	for (i=invSlot.GetNumInList()-1; i>=0; i--)
        {
			...
			if (item && !CanUse(item) && item->HasFlag( ITM_FLAG_ACTIVE ))
            {
		    	UnequipItem(item);
                DisplayCannotUse();
                finished = false; // Teste alles nochmal; vielleicht gibt es Kettenabhängigkeiten?
		    }
	    }
...
/* ----------------------------------------------------------------------
	...
	13.11.2000	[Moos]	
                Dazu: Überprüfe die Benutzbarkeit aller anderen Items
...
void oCNpc :: AddItemEffects (oCItem* item)
{
	...
	// d) Skript ?
	// [EDENFELD] 1.09 NPCs können nun auch Items per Skript equippen (z.B. Ringe etc.)
	if (/*this->IsSelfPlayer() &&*/ (item->on_equip>0)) {
		parser.SetInstance	("SELF",this);
		parser.CallFunc		(item->on_equip);
	}

    CheckAllCanUse();	//  [Moos] Überprüfe alle Items, ob sie noch getragen werden dürfen
}
...
// Check bei Drehschlägen auf "best"-plaziertesten Angreifer
...
// a) in Kampfdistanz und kürzester Winkel
// b) kürzester Winkel
{
	const zREAL maxangle = 360.0f;
...
zBOOL oCNpc :: FocusCheckBBox(const zCVob* vob, const int type, const int last_type, const zREAL low_azi, zREAL& azi)
{
	...
	do {
		...
		if (oCNpcFocus::focus->IsInAngle(type,azi,elev)) {
			// f) Winkeltest = nur bei gleicher Priorität
...
zBOOL oCNpc :: FocusCheck(const zCVob* vob, const int type, const int last_type, const zREAL low_azi, zREAL& azi)
{
	...
	// ab 1.20:     aber nicht für NSCs mit sehr grosser BBOX (derzeit nur Drachen und Trolle)
	zBOOL bigNSC = FALSE;
...
//	vobList.DeleteList	();
	...
	for( int i = 0; i < noofobj; i++ )
	{
		...
		if( (type==zVOB_TYPE_NSC) || (type==zVOB_TYPE_ITEM) || (type==zVOB_TYPE_MOB) )
		{
			...
			// Fokussystem: getötete NSCs/SCs sollten im Melee- und Normalmodus wie MOBs behandelt werden. 
...
// Ist NSC einsatzfähig ? Nein, wenn
// a) bewusstlos
// b) tot
// c) versteinert (Magie)
{
...
// Hat dieser NSC ein Item an, was "npc" gehört
{
	oCItem *item;
	// Waffe testen
	item = GetWeapon();
...
// Jede NSC-Aktion abbrechen !
{
	...
	if (mag) 
	{
		// Der Spieler oder NSCs mit gesetztem override-Flag dürfen nicht den aktuellen Spell abbrechen, wenn sie getroffen werden
	...
	if (GetInteractMob()) {
		...
		if (endMobInteract) 
		{
			// folgender Code spult die Ani zurück, anstatt sie hart zu plöppen
			// States zurück
	...
	// *************************************
	// Alle Anis auf höheren Layer stoppen
	// *************************************
    if (GetVisual())
    {
	...
	// *************************************
	// Und weitere Vars zurück 
	// *************************************	
	ac->ResetAutoRoll();

	// FIXME: folgende zeilen machen bei mobsi benutzung probleme: liegt jemand im Bett und wird getroffen, so plöppt er _immer_ aus dem
	// bett. Seiteneffekte noch unklar
	// Update 1.21: Nun sind die Seiteneffekte klar:
	// Es kann hier Anihänger geben. Da dieses Fragement nur auskommentiert wurde, weil Typen aus dem Bett plöppen, wird
	// der Code nicht aufgerufen, wenn der Bodystate BS_LIE ist
	// zum kotzen, das Standup während des Waffeziehens muss an dieser Stelle _IMMER_ drinbleiben
	// gibt zu viele Probleme ansonsten. Einzige Möglichkeit, die Waffe garantiert ziehen zu können, ist den Aufruf dieser Interrupt()
...
// Kalkuliere Schaden dieser Waffe und berücksichtige die Resistenzen
{
	if (!weapon) return 0;
	int hp	= 0;
	int dam	= 0;
...
// NSC stirbt
{
	...
	// Jetzt mal mit Brennen aufhören
	StopBurn();
...
// c) anderer Vob
{
	...
	// [BENDLIN] Addon Patch2 - bei AI_Teleport wurde SELF überschrieben
...
zBOOL oCNpc :: IsSameHeight(zCVob* other)
{
	...
	// Höhe der Boundingboxes ermitteln
...
// wird nur für den spieler aufgerufen, für WeaponMode>NONE und wenn pariert wird
void oCNpc :: TurnToEnemy()
{
...
void oCNpc :: InitModel()
{
	...
	if (model) {
		...
		// Rüstung angezogen, dann diese Meshlib benutzen
...
//		zERR_ASSERT(selfDist>0);
    ...
    // fackeloverlay?
	// [EDENFELD] ab 1.25 interessieren wir uns nicht mehr für die Fackeloverlay Anis
...
{
	// Starte Gesichtsanimation in Abhängigkeit von der Attitude zum NSC
	zSTRING s;
	switch (attitude) {
...
void oCNpc :: SetWeaponMode2(int fm)
{
	...
	// Toggle den Waffenmodus - dazu gehört :
	// a) Animationen für Waffenmodus initialisieren 
	// b) SlotWechsel der Waffen durchführen
	...
	if (fmode==NPC_WEAPON_NONE) {
		...
		// Waffe ziehen im Rennen ermöglichen
		oCAniCtrl_Human* ac		= GetAnictrl();
		zCModel* model			= GetModel();
		...
		// Und graphischen Wechsel durchführen
		...
		// Waffe wegstecken im Rennen ermöglichen
		oCAniCtrl_Human* ac = GetAnictrl();
		zCModel* model		= GetModel();
		zBOOL walking		= FALSE;
		if (ac) walking		= ac->IsWalking() || model->IsAniActive(ac->_t_walkl_2_walk) || model->IsAniActive(ac->_t_walkr_2_walk);
...
/* ----------------------------------------------------------------------
	...
	13.11.2000	[Moos]	
                Müll (?) beim unequippen beseitigt 
...
//		slot = NPC_NODE_LONGSWORD;
		...
		// Einhänder, nur Waffenslot räumen
		UnequipItem( GetSlotItem(NPC_NODE_SWORD) );
		UnequipItem( GetSlotItem(NPC_NODE_LONGSWORD) );		
//		slot = NPC_NODE_SWORD;
...
//		item -> SetFlag ( ITM_FLAG_ACTIVE );
		...
		if (ac) {
			...
			// Wird benötigt -> CheckActiveSpells
...
/* ----------------------------------------------------------------------
    ...
    aus der rechten Hand oder dem Inventory genommen, angezündet und
...
//changed[dennis]
		...
		// **************************************
		// a) Fackel anzünden und in Player-Hand
		// **************************************
...
//			if (torch) SetRightHand(torch);
		...
		{
            // lösche eine Fackel in der linken hand
            TNpcSlot *slotLeft = GetInvSlot(NPC_NODE_LEFTHAND);
            ...
            else  // Fackel in der rechten Hand?
            {
                ...
                {
                    vob->AddRefVobSubtree();	//  [Moos] Sonst werden Teile der Fackel gelöscht
...
// NEW
//[EDENFELD] AddRefs auf die spells durchgeführt
...
// Ermittelt Anzahl der Alchemie Sprüche
{
	//changed[dennis]
...
{
    SetWeaponMode2(NPC_WEAPON_NONE);	// [Moos] Waffe wegstecken, sonst ärger (equippte Waffe, aber nicht im Slot).
		...
		if (!invSlot[i]->inInventory && vob) {
			...
			if (oWld){
                if (!oWld->GetVobList()->IsInList(vob))
                    oWld->InsertVobInWorld(vob);  // [Moos] ist nach Levelwechsel manchmal nötig
...
//	state.ClearParserReferences();
	...
	// Die Verweise auf temp. Objekte löschen
	ClearFocusVob	();
	ClearVobList	();
	...
	// Und Pointer auf Cutscenes löschen
	state.CloseCutscenes();

	// InteractItem löschen
	SetInteractItem	(NULL);
...
{	
	...
	return (voiceIndex==0);	// TRUE, wenn nächster Aufruf werder am Anfang beginnt
};
...
void oCNpc :: ProcessNpc()
{
    ...
    // [Moos] Kümmere dich um geplante Fackelanimationen
    CheckSetTorchAni();
...
void oCNpc :: PrintStateCallDebug(const zSTRING &name, int callfunc, int callperc)
{
	...
	// Callfunc = -99 -> Keine Skriptfunktion nötig
	if (callfunc!=-99) {
		zCPar_Symbol* sym = NULL;
...
/* ----------------------------------------------------------------------
	...
	22.09.2000	[Moos]	
                Optimiert (==komplett umgestellt) und Distanzparameter eingefügt
...
// TYPES = Attack / Steal / Aims
{
	...
	for (int i=0; i<max; i++) {	
		...
		if ((v->GetVobType()==zVOB_TYPE_NSC) && (v!=victim) && (v!=this)) {
			// NSC, der die Sitution beobachtet haben könnte, gefunden
			oCNpc *n = (oCNpc *)v;
			...
			// *** New ***
			// Nur LOS-Check, so können alle Beteiligten NSC in der Nähe darauf reagieren
			// ***********
...
// Basierend auf letzen Warnehmungscheck 
{
	...
	for (int i=0; i<max; i++) {	
		...
		if (npc) {
			...
			if ((ignorePlayer)	&&  npc->IsSelfPlayer())				continue;				
			if ((ignoreDead)	&&  !npc->IsConditionValid())			continue;				
			// [BENDLIN] 2002-05-12, nur die OTHER, die in einem bestimmten Verhältnis zu SELF stehen (nicht umgekehrt)
...
// OTHER	= enemy
{
	...
	// dann NEIN,
	// sonst JA (= wir SIND stärker, nicht echt, aber ab doppelter Gegnerstärke
	// zu fliehen ist früh genug, sonst kommen die Jungs aus dem Rennen nicht
	// mehr raus :) )
	// Beachten:
	// 1) Monster können feindlich zu beiden Menschen sein --> egal, zählen auf
	// beiden Seiten
...
// Keine Rüstung  generiere Rüstung nach Tabelle
...
// LE, Mana und PSI auf Max
{
	...
	if (!item) {
		// generiere Rüstung aus Tabelle.... ne, besser aus Skript
		parser.SetInstance	("SELF",this);
...
// liefert 0 - ungültig
// 1 - 4 für PlayerInstanzen identifiziet über deren Instanzname !
{
...
void oCNpc :: SetAsPlayer()
{
	...
	if (player) {
		// Der alte Player löscht RefenceVerweise
		player->ClearVobList	();
	...
	for (int i=0; i<c; i++) {
		...
		if (ev->aniEventType==zMDL_EVENT_TAG) {			
			if (ev->tagString=="DEF_FIGHTMODE") {
				...
				// **************************************
				// Item Interaktion - Item einfügen
				// **************************************
//				zCView::StdPrintwin(zSTRING("Send Insert\n"));
				...
				// **************************************
				// Item Interaktion - Item zerstören
				// **************************************
...
//				zCView::StdPrintwin(zSTRING("Send Insert\n"));
				...
				if( item && (item->GetInstanceName().Search("ITLSTORCH")==0) )
				{
                    ...
                    // [Moos] 18.1.2001: lasse die Fackel bei nächster Gelegenheit wieder holen:
...
// Füge Vob ins Inventory / Slot ein 
{
	if (!vob) return FALSE;
...
// Entferne Vob aus Inventory und lasse es fallen
{
	...
	if (vob->GetVobType()==zVOB_TYPE_ITEM) 
	{
		...
		// In der Hand ? dann diese Löschen ?
		...
		if(item->HasFlag(ITM_FLAG_ACTIVE)) 
		{
			...
			// fixt einen Bug der sich darin äusserte, daß versehntlich zweimal die Itemboni beim 
...
{		
    ...
    zBOOL wasTree = SafeAddRef(vob);	//  [Moos] Crashfix; beim werfen der Fackel wird sonst beim Entfernen aus der zCWorld der VobTree gelöscht
...
// 
{
	// Im Magiemodus - böser Zauberspruch und am aimen
	...
	if (GetWeaponMode()>=NPC_WEAPON_MAG) {
		...
		if (spell && /*spell->InCast() &&*/ (spell->GetCategory()==SPL_CAT_BAD)) {
			// Spruch ist böse, und es wird geaimt.
			return TRUE;
		}
	}
...
void oCNpc :: Disable()
{
    ClearVobList();
 	...
 	if (GetHomeWorld())
        AvoidShrink();      // [Moos] sorge dafür, dass er ein Visual hat.
...
void oCNpc :: Enable(zVEC3& pos)
{
    ...
    GetModel();  // [Moos] sorge dafür, dass das Model VOR der Positionssuche da ist.
	...
	// Position setzen
	SetCollDet			(FALSE);
	SearchNpcPosition	(pos);				// Setzt Höhe und checkt auf Platz.
...
void oCNpc :: Archive (zCArchiver &arc)
{
	...
	// [MOOS] speichere das Visual zwischen und lösche es aus der Datenstruktur. Es muss nicht gespeichert werden.
	...
	// alles veränderliche jetzt drüberlegen
	...
	// [BENDLIN] damit er sich merkt, wieviel Mana für den Spruch auf der Hand investiert werden soll
...
*/
    ...
    if (GetAI())
        AvoidShrink();  //[Moos] Das hier ist der frühestmögliche Zeitpunkt, das Model zu restaurieren.
...
//	arc.ReadInt		("guildTrue"	,guildTrue);
	...
	for (int i=0;i<numTalents;i++)
	{
		...
		if (talent)
		{
			// [EDENFELD] 1.27a Workaround für korrupte Savegames 
...
//			}
	...
	if (ai) 
	{
        AvoidShrink();    // [Moos] sorge dafür, dass die AI ein Model zum Spielen hat
	...
	if (isPlayer && !oCNpc::player) {
		// Gibt's den Spieler nocht nicht, diesen übernehmen
		oCNpc::player = this;
	}
...
//	arc.ReadBool	("respawn"		,respawnOn);
    ...
    if (GetVisual())   InitModel();	               // [Moos] damit eine Rüstung GAAANZ sicher angezogen wird.
...
//	zERR_ASSERTT(item->GetRefCtr()==2,"U: INV: Item-Reference-Counter != 2");
	...
	if (HasTalent(NPC_TAL_IDENTIFY) && (item->GetInstanceName()=="ITFOMUSHROOM")) {
		// NOTE : OriginalItem wird gelöscht !!!!	
		IdentifyAllMushrooms();
	}
	*/

    zRELEASE(item);  // [Moos] gegenstück zum AddRef ganz oben
...
void oCNpc :: DeleteInvSlot (const zSTRING& name)
{
	...
	for (int i=0; i<invSlot.GetNumInList(); i++) {
		...
		if (slot->name==name) {
			invSlot.Remove(slot);
//            zRELEASE(slot->vob);  // CODECHANGE Moos: Leakfix ; [Moos, viel später] macht jetzt der Destruktor
			delete(slot);
			return;
		}
	}
...
void oCNpc :: PutInSlot (TNpcSlot* slot, oCVob* vob, zBOOL inInv)
{
	...
	if (!IsSlotFree(slot))
    {
        ...
        if (slot->Vob()->GetObjectName() == S_ITLSTORCHBURNING && slot == GetInvSlot(NPC_NODE_LEFTHAND) &&
            ...
            GetActiveInfoWritablePrivate()->putTorchBackInHand = TRUE;  // räume sie bei Gelegenheit wieder weg.
        }
	...
	{
		// Falls nicht in Welt, das Teil einfügen
		vob -> SetCollDet (FALSE);	
		if (!vob	-> GetHomeWorld()) {
...
/* ----------------------------------------------------------------------
	...
	20.10.2000	[Moos]	
                Erzeugt, um das Fackelleck zu stopfen (so kann oCWorld die npc-Slots aufräumen)
...
oCVob* oCNpc :: DropFromSlot(TNpcSlot* slot)
{
	...
	if (slot->inInventory) 
	{
		...
		{
			// ok, gib dem vob wieder die kontrolle über sich selbst
    ...
    */
	...
	// FIXME : Ist das hier wirklich nötig ? 
...
void oCNpc :: UpdateSlots()
{
	...
	for (int i=0; i<invSlot.GetNumInList(); i++) 
	{
		...
		{			
			...
			// beim laden eines savegames kann ein item-effekt nocht nicht als child an den npc gehängt werden
...
{
	// FIXME : Safety, nicht unbedingt nötig
			...
			for (int i=0; i<invSlot.GetNumInList(); i++) {
				...
				if (slotItem) {
					...
					if ((slot->tmpLevel==0) && (slotItem->GetInstance()==interactItem->GetInstance())) {
						...
						// verschwand.
						// Man könnte hier auch das neue Item wieder in den Slot stecken, aber aus Sicherheitsgründen
						...
						// ansonsten wird Welt gerade abgeräumt
...
void oCNpc :: OnMessage	(zCEventMessage *eventMessage, zCVob* sourceVob)
{
    ...
    if (!GetVisual() || !GetAnictrl())
        AvoidShrink();	//  [Moos] GRRR. Falls mal ein nicht in der Welt eingefügter NSC eine Nachricht erhält...
	...
	// Hier Addref drauf, damit message nicht Irgendwo gelöscht wird (zB. Cutscene-Clear)
	eventMessage->AddRef();
	...
	if (zDYNAMIC_CAST<zCEvMsgCutscene>(eventMessage))
	{
		...
		switch (csg->GetSubType())
		{
			...
			/*
				...
				//	SCREEN LOESCHEN
				//
                if (ogame && ogame->GetView(oCGame::GAME_VIEW_SCREEN)) // [Moos] Crashfix; im Destruktor von oCGame sind die Views schon gelöscht...
...
//			case oCMsgManipulate::EV_REMOVEFROMSLOT	:	del = EV_RemoveFromSlot	(csg);		break;
	...
	if (zDYNAMIC_CAST<oCMsgState>(eventMessage)) {
		...
		switch	(csg->GetSubType()) {
			case oCMsgState::EV_STARTSTATE		:	// Hier wird eh immer der EM gelöscht,
	...
	if (!csg->initDone) {
		...
		if (model->IsAniActive(anictrl->_s_walkl)) {
			...
			if (csg->ani!=zMDL_ANI_ID_ILLEGAL) {
				csg->duringRun = TRUE;
				// NachfolgeAni NULL, weil Teilkörper Ani auf Layer 2
		...
		// **********************************
		// Turnings für den Spieler erlauben
		// **********************************
...
zBOOL oCNpc :: EV_RemoveWeapon1(oCMsgWeapon* csg)
{
	...
	if (anictrl->RemoveWeapon1()) {	
		if (IsAPlayer()) {		
			// Leertaste noch gedrückt, dann Magiekranz starten
	...
	else 
	{
		...
		if( IsSelfPlayer() )
		{
			...
			if (!oCGame::GetUseOldControls())
			{
				...
				// wurde während des Combo Fensters die Up Taste gedrückt, so kann man weitergehen
				if( zinput->GetState(GAME_UP) && zinput->GetToggled(GAME_UP) && (anictrl->IsInCombo() || anictrl->IsInPostHit()) )
				{
					model->StartAni(anictrl->_s_walk);	
					return TRUE;
				}
				// wurde während des Combo Fensters die Down Taste gedrückt, so kann man weitergehen
		...
		else 
		{
			...
			{
				// Ansonsten gibt Combo Anzahl der combos an, die gespielt werden dürfen...
				anictrl->HitCombo(TRUE);
	...
	if (interactItem) {
		...
		if (GetSlotItem(csg->slot)!=interactItem) {
            ...
            if (IsInInv(interactItem))
			    SetInteractItem	(RemoveFromInv(interactItem,1));	//  [Moos] 2.2.2001: Das InteractItem ist aber doch gar nie im Inventory! Das gibt Ärger....
...
zBOOL oCNpc :: EV_CreateInteractItem	(oCMsgManipulate* csg)
{
	...
	// items aus dem MDS erzeugt (Hämmer, etc.)
	// FIXME: ich weiss hier nicht was ich tue, so werden zwar die hämmer korrekt dargestellt,
	// (und ich mach auch mal ein interact item daraus)
	// aber eventuell gibt es item verdoppler crashes, fehlende Hämmer beim Laden, etc...
...
zBOOL oCNpc :: EV_UseItem(oCMsgManipulate* csg)
{
	...
	if (EV_UseItemToState(csg)) {
		...
		if (csg->targetState>= 0) {
			...
			// und der gewünschte loop ani state erreicht ist.
			// z.B. ist EV_UseItemToState() beendet, wenn T_POTION_STAND_2_S0 beendet ist, und S_POTION_S0 angespielt wird
			// ob das von der Implementation so gewünscht ist, kann ich nicht sagen.
			// das SetInUse(FALSE) war hier dafür gedacht, das bei einem nächsten Call von EV_UseItemToState() der neue targetState (=zurückspulen)
			// übernommen wird. 
			// das ist jetzt nicht mehr nötig, da der state nun jeden Frame mit übernommen wird.
...
{
	if (!csg->IsInUse()) {
		// Ist der Vob überhaupt ein Item ?
		oCItem* item = zDYNAMIC_CAST<oCItem>(csg->targetVob);
		if (!item) {
			...
			// wenn bereits in Welt per "DEF_INSERTITEM" eingefügt
...
//		}
	...
	if (InteractItemIsInState(csg->name, interactItemCurrentState)) {		
		...
		// Direkter Wechsel zum Zielzustand möglich ?
		zCModel* model	= GetModel();
		...
		if (ani) {
			...
			// Kein Direkter Wechsel möglich, also gehen wir Schrittweise
			int dir = 0;
			if (interactItemCurrentState < interactItemTargetState) dir = +1;
			else													dir = -1;
			// Wechsel zum angestrebten nächsten Zustand möglich ?
...
zBOOL oCNpc :: ApplyTimedOverlayMds( const zSTRING& mdsName, zREAL timer )
{
	...
	//	START MDS
	//
	pModel	->ApplyModelProtoOverlay	(mdsName);												// [Moos] dass hier nicht ApplyOverlay() aufgerufen wird, ist OK; es ist ja nur ein temporäres Overlay.
...
{	
	...
	// Über Namen die Nummer ermitteln
	// AI - Output - checken
...
zBOOL oCNpc :: EV_PlaySound(oCMsgConversation* csg)
{
	...
	{	
		...
		// Text nur anzeigen, wenn Entfernung nicht größer als 5m
...
void oCNpc::AddEffect	(const zSTRING& a_strNewFX, const zCVob* a_pTarget)
{
	...
	// checken, ob bereits so ein effekt läuft
	for (int i=0; i<effectList.GetNum(); i++)
	{
		oCVisualFX* newFX = effectList[i];
...
void oCNpc::RemoveEffect(const zSTRING& a_strFX)
{
	...
	// checken, ob überhaupt so ein effekt läuft
	for (int i=0; i<effectList.GetNum(); i++)
	{
		oCVisualFX* newFX = effectList[i];
...
zBOOL oCNpc :: EV_LookAt(oCMsgConversation* csg)
{
	...
	// B_LookAt beinhaltet das Drehen des Körpers, 
	// aber nur in Körperhaltungen, deren Animationen 
	...
	if (!csg->IsInUse()) {
		...
		for (int i=0; i<GetEM()->GetNumMessages(); i++) {
			...
			if (convMsg) {
				if (convMsg == csg) {
					// Nur Sachen löschen, die schon hinter mir liegen...
		...
		if (!csg->target) {
			// Kein Target angegeben, über Vobnamen ermitteln
		...
		// StoplookAt aufrufen, falls nötig
		if (  ( csg->target != NULL ) 						&&
...
zBOOL oCNpc :: EV_PointAt(oCMsgConversation* csg)
{
	...
	// Wie beim Ansehen ist es auch möglich, 
	// NSCs situationsbezogen auf etwas zeigen zu lassen. 
	// Dies funktioniert aber nur für humanoide Wesen (Menschen, Orcs, evtl. Gobbos). 
	// Wie bei Quicklook zeigt der NSC nur kurz!
	// Nur bei Anis mit FreeHands-Flag möglich!
...
zBOOL oCNpc :: EV_StopProcessInfos( oCMsgConversation* csg )
{
	...
	// [EDENFELD] 1.09: der npc sollte auch wissen das das gespräch beendet ist.
...
// FIXME	: Other wird so nicht mehr benötigt
{
...
// Magie wird auf "self" von "other" ausgeübt
{
...
{
	if (!IsSelfPlayer()) return FALSE;
	// Passives Sehen. Wird ausgesendet, sobald ein SC seine Rüstung wechselt 
	// oder sich verwandelt (nicht bei Rückwandlung oder ausziehen!)
...
// eigene passive assessUseMob-Wn bei Sc fummelt anTür/Truhe/Schloß, 
// rum aber auch sitzt auf Stuhl etc.
{
	if (!mob)				return FALSE;
	if (!IsSelfPlayer())	return FALSE;
	// aktives Mob in rbt-Struktur setzen und so den Zugriff über Build-Ins damit aktivieren.
	rbt.obstVob = mob;
...
// Erstelle Liste umgebender NSCs - für passive Wahrnehmung
{
	if (!GetHomeWorld()) return;
...
//		vob -> AddRef();
	...
	for (i=0; i<percActive; i++) {
		...
		// Falls eine Wahrnehmung keine weitere mehr zulässt, müssen diese
...
{	
	...
	// ******************************************************
	// a) Werte für Dauer der temp-Attituden ANGRY / ANNOYED 
...
{
	// das item hängt entweder: a) als tree im worldspace
	// b) als child am npc
	// c) nirgendwo
		...
		{			
			...
			if ( (NULL!=a_pItem->GetEffect()))
			{
				...
				else if (a_bModelControl)
				{
					...
					if (!a_pNode) 
					{
						// sonderfall rüstungen: im gegensatz zu slot-items wzb. waffen 
						// repräsentiert die (angezogene) rüstung das komplette softskin und wird nicht als node-visual gerendert
						// daher müssen wir dem effekt nun sagen, das das pfx-visual nun das model ist
					...
					// FIXME: während des ladens eines save-games ist hier der vobtree noch nicht bekannt
					// lösung: effekt aus der welt entfernen und in updateslots später an den parent
					// linken

...
{
	// Tagesablauf aller NSCs mit angegebenen Namen ändern
...
{
	// FIXME : Bugfix fuers einfügen von NSCs im Spacer,
...
// CODECHANGE [MICHAEL]
// diese Funktionen sind offenbar Überreste vergangener Zeiten
...
static zBOOL Npc_ClearInventory()
{
	...
	if ( (NULL != pNpc) && !pNpc->inventory2.IsEmpty() )
	{
		...
		// Davon ausgehen, dass ALLE items gelöscht werden müssen
		int itemsToBeLeft = 0;
		...
		while (pNpc->inventory2.GetNumItemsInCategory() > itemsToBeLeft)
		{
			...
			if (pItem)
			{
				...
				if ( anz > 0 )
				{
					...
					{	// dann die Abbruchbedingung ändern
						itemsToBeLeft++;
					}
...
static zBOOL EquipItem()
{
	...
	if (npc && ogame && !ogame->IsInLoadSaveGame()) {
		...
		if (item)
		{
			...
			// [EDENFELD] 1.09 auch besondere Qualitäten des (magischen) Items mit aktivieren (mehr Health z.B.)
...
static zBOOL AI_RemoveWeapon()
{
	...
	// [EDENFELD] 1.09: NPC mit gezogener Fernkampfwaffe hört bei AI_RemoveWeapon automatisch
	// auf zu zielen
	if (self)
	{
		...
		{
			// npc hat fernkampfwaffe gezogen. Hör doch mal auf zu zielen
...
// IsAvailableMission ( self, state )
{
	...
	if (npc) 
	{
		...
		// a) MIS_NOTINVOLVED, aber Kriterien erfüllt
		// b) MIS_RUNNING, Mission läuft

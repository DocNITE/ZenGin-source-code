...
*****************************************************************************************************
  ...
  .ASC Materialien:
	...
	*MESH_FACE_LIST {
	...
	Falls ein Material SubMats hat, zaehlen nur die SubMats. Das übergeordnete 'Multi-Material' hat
	keine Bedeutung
...
//    for(i=pfxEmitterList.GetNum()-1;i>=0;i--) // Die Emitter löschen sich doch selbst, also ist das hier nicht nötig
...
//				mat->SetSpecular	(0.0F);
			...
			// es handelt sich um ein MultiMat => das übergeordnete Material wird nicht gebraucht
			mat->Release();
			mat = 0;
		...
		case zFCHUNK_MDS_ANI:
			{
				...
				// Beachte: was wenn ASC-Ani dieselbe bleibt, aber die Params im MDS sich ändern ?
				if (aniPtr) {
...
zBOOL zCModelPrototype::LoadModelScript (const zSTRING& filename) 
{
...

	{
		...
		zoptions->ChangeDir(DIR_ANIMS);  // sonst wird Datei nicht gefunden - nötig?

...
- Kommentare:
	- Vor dem Zugriff auf ein Desktop-Item dieses ab sofort immer 
	  erst in den Vater einfügen : z.B. screen->Insert(View). 
	  Erst danach Grafikzugriffe auf "View" durchführen.
	  ...
	  Wurde die aktuelle Taste vom Eventhandler abgearbeitet, muß TRUE 
	  zurückgeliefert werden (da sonst die Taste an evt. weitere Handler 
...
// [Moos] : statische Aufräumfunktion für obige Pointer
//          Kein toller Stil und abhängig vom Compiler, aber auch unkritisch.
...
void zCView :: RecalcChildsPos()
{
	...
	// nue neue Berechnung der PixelPositionen nötig
	zCView* child = childs.wurzel;
	while (child) 
	{
		child -> pposx = 0;
...
// Liefert Item an Pos x/y zurück
{
	int x1,y1,x2,y2;
...
int zCView :: FontY()
// Liefert max.Höhe des aktuellen Fonts
{
...
// Liefert Länge des Strings in virtuellen Koords
{
	if (font) 
...
void zCView :: Nextline(const zSTRING& s, zREAL fTime, zCOLOR* col, int* offsetY )
{
	...
	if (winy + FontY() > py2) 
	{
		...
		while (textNode) 
		{
			...
			{
				textData->posy -= FontY();
				// Falls schon unter 0, dann ganz löschen.....
